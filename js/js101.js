	...
	if(data.length){
		data.forEach(function(item){
			...
			if(item.username == index){
				...
				container.querySelectorAll("img")[0].setAttribute("src", "images/avatar/" + item.avatar + ".png");
				//container.querySelectorAll("p")[0].innerHTML = item.description;

			 var superp = document.getElementById("description");
             var dream = item['summer dream']
...
function renderFilterResults(filters, profiles){
	...
	var criteria = ['JS101 interests', 'Other programming'],
		results = [],
		isFiltered = false;
...
function buildprofileList(data){
   ...
   for(var i = 0, n = data.length; i < n; i++ ){
   	 ...
   	 image.setAttribute("src", "images/avatar/" + obj.avatar + ".png");
